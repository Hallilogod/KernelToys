...
BOOL CreateKeyToyHandler(LPWSTR arguments[], UINT argumentCount)
{
    ...
    return DeviceControlDriver(IOCTL_CREATE_KEY, arguments[1], (wcslen(arguments[1]) + 1) * sizeof(WCHAR), NULL, 0);
}
...
BOOL DeleteKeyToyHandler(LPWSTR arguments[], UINT argumentCount)
{
    ...
    return DeviceControlDriver(IOCTL_DELETE_KEY, arguments[1], (wcslen(arguments[1]) + 1) * sizeof(WCHAR), NULL, 0);
}
